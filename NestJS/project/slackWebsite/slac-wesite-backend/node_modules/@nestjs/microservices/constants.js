...
export const RMQ_NO_EVENT_HANDLER = (text, pattern) => `An unsupported event was received. It has been negative acknowledged, so it will not be re-delivered. Pattern: ${pattern}`;
export const RMQ_NO_MESSAGE_HANDLER = (text, pattern) => `An unsupported message was received. It has been negative acknowledged, so it will not be re-delivered. Pattern: ${pattern}`;
export const GRPC_DEFAULT_PROTO_LOADER = '@grpc/proto-loader';
export const NO_EVENT_HANDLER = (text, pattern) => `There is no matching event handler defined in the remote service. Event pattern: ${pattern}`;
...
export const BLOCKED_RMQ_MESSAGE = (reason) => `RMQ broker has blocked the connection (flow control). Reason: ${reason}`;
