...
export class ClientRedis extends ClientProxy {
    ...
    subscriptionsCount = new Map();
    pubClient;
    subClient;
    connectionPromise = null;
    isManuallyClosed = false;
    wasInitialConnectionSuccessful = false;
    ...
    async createClient() {
        ...
        const redisPackage = await loadPackage('ioredis', ClientRedis.name, () => import('ioredis'));
    ...
    registerReadyListener(client) {
        client.on("ready" /* RedisEventsMap.READY */, () => {
            ...
            if (!this.wasInitialConnectionSuccessful) {
                ...
                this.subClient.on(this.options.returnBuffers ? 'messageBuffer' : 'message', this.createResponseCallback());
            }
        });
    }
    ...
    createResponseCallback() {
        return async (channel, buffer) => {
            ...
            try {
                ...
                this.logger.debug('Redis response packet is not in json format, bypassing...');
                packet = buffer;
            }
            ...
            if (!callback) {
                if (Buffer.isBuffer(buffer))
                    this.logger.debug('You have to parse your buffer on your own to get id from it, because it is not in json format');
                this.logger.debug('No matching callback found for Redis response packet with id: ' + id);
                return;
            }
            if (isDisposed || err) {
                return callback({
