...
import { EMPTY, ReplaySubject, Subject, Subscription, defaultIfEmpty, fromEvent, lastValueFrom, } from 'rxjs';
...
export class ServerGrpc extends Server {
    ...
    constructor(options) {
        ...
        grpcPackage = this.loadPackageSynchronously('@grpc/grpc-js', ServerGrpc.name, () => createRequire(import.meta.url)('@grpc/grpc-js'));
        grpcProtoLoaderPackage = this.loadPackageSynchronously(protoLoader, ServerGrpc.name, () => createRequire(import.meta.url)(protoLoader));
    }
    ...
    async createService(grpcService, name) {
        ...
        for (const methodName in grpcService.prototype) {
            ...
            if (!isUndefined(methodReqStreaming) && methodReqStreaming) {
                ...
                methodHandler = this.getMessageHandler(name, methodName, GrpcMethodStreamingType.RX_STREAMING, methodFunction);
                ...
                if (!methodHandler) {
                    methodHandler = this.getMessageHandler(name, methodName, GrpcMethodStreamingType.PT_STREAMING, methodFunction);
                    streamingType = GrpcMethodStreamingType.PT_STREAMING;
                }
            }
            else {
                // Select handler if any presented for No-Streaming pattern
                methodHandler = this.getMessageHandler(name, methodName, GrpcMethodStreamingType.NO_STREAMING, methodFunction);
    ...
    createUnaryServiceMethod(methodHandler) {
        return async (call, callback) => {
            return this.onProcessingStartHook(this.transportId, { ...call, operationId: methodHandler.name }, async () => {
        ...
        return async (call, callback) => {
            return this.onProcessingStartHook(this.transportId, { ...call, operationId: methodHandler.name }, async () => {
                ...
                if (isResponseStream) {
                    ...
                    const response = await lastValueFrom(res.pipe(takeUntil(fromEvent(call, CANCELLED_EVENT)), catchError(err => {
                        callback(err, null);
                        return EMPTY;
    ...
    loadProto() {
        try {
            const packageDefinition = getGrpcPackageDefinition(this.options, grpcProtoLoaderPackage);
    ...
    collectDeepServices(name, grpcDefinition, accumulator) {
        ...
        for (const key of keysToTraverse) {
            ...
            const isFormatDefined = deepDefinition && !isUndefined(deepDefinition.format);
    ...
    bufferUntilDrained() {
        ...
        let replayBuffer = new ReplaySubject();
